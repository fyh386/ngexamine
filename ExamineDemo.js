...
function Examine() {
    ...
    this.ExaminerSubmit = function () {
        ...
        $.post(_this.ExaminerSubmitUrl, _this.SubmitArg
            , function (r) {
                if (r.code == 0) {
                    layer.alert(r.message, { icon: 2, title: "错误" });
                    layer.close(_this.CurrentFrameIndex);
                    return;
                } else if (r.code == 1) {
                    //如果不通过也返回
    ...
    this.InitiatorSubmit = function () {
        ...
        _this.SubmitArg = $.extend(true, _this.SubmitArg, { status: parseInt($("[name=radio1]:checked").val()), content: $("#examine_content").val() });
        $.post(_this.InitiatorSubmitUrl, _this.SubmitArg
            , function (r) {
                if (r.code == 0) {
                    layer.alert(r.message, { icon: 2, title: "错误" });
                    layer.close(_this.CurrentFrameIndex);
...
//实例化表格对象后期改为非依赖形式
var EjyGrid = new jqGrid("/NewExamine/GetExamineHistory");
