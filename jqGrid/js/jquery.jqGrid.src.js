...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		endReq = function() {
			...
			switch(ts.p.loadui) {
				case "disable":
					break;
				case "enable":
				    $("#load_" + $.jgrid.jqID(ts.p.id)).fadeOut("normal");
					break;
				case "block":
				    $("#lui_" + $.jgrid.jqID(ts.p.id)).fadeOut("normal");
				    $("#load_" + $.jgrid.jqID(ts.p.id)).fadeOut("normal");
					break;
			}
		},
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			else { ts.p.colModel[i].width = parseInt(ts.p.colModel[i].width, 10) + 15; }
		...
		firstr = null;
        //UPDATE：table width+18px;
		var hTable = $("<table class='ui-jqgrid-htable' style='width:"+(ts.p.tblwidth+19)+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
		...
		$(grid.hDiv)
			...
			.addClass("ui-state-default ui-jqgrid-hdiv" + (ts.p.hideHeader ? " ui-hidden" : ""))
			.append(hb);
		$(hb).append(hTable);
		hTable = null;
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({ width: grid.width + "px" }).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom' + (ts.p.hideFooter ? ' ui-hidden' : '')).appendTo(eg);
		...
		$(grid.bDiv)
            ...
            //UPDATE：overflow-x:hidden
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width)+"px","overflow-x":"hidden"})
			.scroll(grid.scrollGrid);
        //UPDATE:table width+18px;
		$("table:first", grid.bDiv).css({ width: ts.p.tblwidth + 19 + "px" });
		if( !$.support.tbody ) { //IE
